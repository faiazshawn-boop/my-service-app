...
import time
import threading # (থ্রেডিং আর ব্যবহার হচ্ছে না, তবে রাখা যেতে পারে)
import math
import random
import os
import re
...
app = Flask(__name__)
bot = telebot.TeleBot(TOKEN, threaded=False) # threaded=False গুরুত্বপূর্ণ
...
# ===== আপনার পুরানো টেবিল জেনারেটর ফাংশন =====
def get_short_data_for_table(order_data):
    ...
    if not nid and not dob:
        nid = order_data.get("বাংলালিংক নাম্বার", order_data.get("গ্রামীন নাম্বার", order_data.get("রবি নাম্বার",
              order_data.get("এয়ারটেল নাম্বার", order_data.get("টেলিটক নাম্বার", order_data.get("MOBILE NO",
              order_data.get("বিকাশ নাম্বার", order_data.get("নগদ নাম্বার", "")))))).strip()
        dob = ""
...
def get_init_data_route():
    # (এটি get_init_data ফাংশনকে কল করবে)
    return get_init_data()

# রুট ২: মিনি অ্যাপ থেকে নতুন অর্ডার রিসিভ করা
@app.route('/submit_order', methods=['POST'])
def submit_order_route():
    # (এটি submit_order ফাংশনকে কল করবে)
    return submit_order()
    
# (ফাংশনগুলো এখানে ডিফাইন করা হলো)
def get_init_data():
    try:
        data = request.json
...
def submit_order():
    try:
        ...
        try:
            # এখানে আপনার নতুন টেবিল ডিজাইন ব্যবহার করা হলো
            order['progress_msg_id'] = -1 # একটি প্লেসহোল্ডার
...
# (প্রোডাক্ট ম্যানেজমেন্ট ফাংশনগুলো)
        ...
        if order_id in orders:
            ...
            if not order_data: hidden_info += "কোনো তথ্য সেভ করা নেই।"
                ...
                for label, value in order_data.items():
                    ...
                    if isinstance(value, str) and value.startswith(("AgAC", "BAAC")): display_value = "(ছবি 🖼️)"
...
# ===== পুরানো অর্ডার ফ্লো ফাংশনগুলো (মিনি অ্যাপ দ্বারা প্রতিস্থাপিত) =====
# (process_order_step, handle_order_inputs, cleanup_chat, handle_final_order_decision, 
# handle_sub_option_callback, handle_confirmation_callback, delete_action_message, handle_text)
# এই ফাংশনগুলো আর দরকার নেই, কারণ সব কাজ /submit_order রুটে হচ্ছে।
# শুধুমাত্র /admin কলব্যাকগুলো রাখা হয়েছে।

# ===== আপডেট বাটন (এখন আর দরকার নেই) =====
@bot.message_handler(func=lambda message: message.text == "🔄 আপডেট")
def handle_update_button(message):
    try: bot.delete_message(message.chat.id, message.message_id) 
    except Exception: pass
    start(message) # শুধু /start কমান্ড আবার চালানো

# ===== অন্য সব টেক্সট ইগনোর করা =====
...
if __name__ != "__main__":
    ...
    try:
        ...
        logger.info("বটের Webhook সেট করা হচ্ছে...")
        bot.remove_webhook()
        time.sleep(0.5)
        bot.set_webhook(url=RENDER_APP_URL + '/' + TOKEN)
        logger.info(f"Webhook {RENDER_APP_URL}-এ সেট করা হয়েছে।")
        
    except Exception as e:
        logger.error(f"Gunicorn চালু করার সময় এরর: {e}")

# (লোকাল টেস্টিং-এর জন্য এই অংশটি রাখা যেতে পারে, কিন্তু Render এটি ব্যবহার করবে না)
if __name__ == "__main__":
